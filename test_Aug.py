...
# Compare baseline vs. basic data augmentation (RandomResizedCrop + Flip + ColorJitter + RandomErasing)
# Requires: torch, torchvision
# Expects: ViT_Tiny_spt_lsa.py (with vit_tiny_patch16_224) in the same directory.
...
def run_experiment(args, preset: str):
    ...
    for epoch in range(args.epochs):
        t0 = time.time()
        tr_loss, tr_top1 = train_one_epoch(model, train_loader, optimizer, loss_fn, device, scaler)
...
def parse_args():
    p = argparse.ArgumentParser("ViT-Tiny Baseline vs Basic Augment Tester")
    # Data
